...
def load_pdf(file_path: str) -> str | None:
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            if total_pages == 0:
                ...
                return None
            print(f"📄 Reading PDF: {os.path.basename(file_path)} ({total_pages} pages)", file=sys.stderr)
            pages = []
            for i, page in enumerate(reader.pages):
                print(f"   Extracting page {i+1}/{total_pages}...", end="\r", file=sys.stderr)
                try:
                    page_text = page.extract_text()
                    if page_text and page_text.strip():
                        pages.append(page_text.strip())
                except Exception as e:
                    print(f"\n⚠️  Warning: Could not extract page {i+1}: {e}", file=sys.stderr)
                    continue
    except PyPDF2.errors.PdfReadError as e:
        print(f"❌ Error: Cannot read PDF (file may be corrupted or encrypted): {e}", file=sys.stderr)
        return None
    ...
    if not pages:
        print(f"\n❌ Error: Could not extract any text from PDF. It may be a scanned/image PDF.", file=sys.stderr)
        print("   PyPDF2 only works with text-based PDFs, not scanned images.", file=sys.stderr)
        return None

    full_text = "\n\n".join(pages)
    full_text = full_text.strip()
    print(f"\n✅ Extracted {len(full_text)} characters from {total_pages} pages", file=sys.stderr)
    return full_text
...
def main() -> None:
    ...
    try:
        ...
        parser.add_argument("file", help="Path to the PDF file to extract")
        parser.add_argument("--chars", type=int, default=500, help="How many characters of output to print (default: 500)")
